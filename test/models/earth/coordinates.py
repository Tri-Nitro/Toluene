...
class TestCoordinates:
    ...
    def test_eci_to_ecef(self):
        for index in range(len(eci_test_points)):
            ...
            assert ((ecef_test.x - ecef_test_points[index].x)**2 + (ecef_test.y - ecef_test_points[index].y)**2 + (ecef_test.z - ecef_test_points[index].z)**2)**0.5 == pytest.approx(0, abs=3)
    ...
    def test_ecef_to_eci(self):
        for index in range(len(eci_test_points)):
            ...
            assert ((eci_test.x - eci_test_points[index].x)**2 + (eci_test.y - eci_test_points[index].y)**2 + (eci_test.z - eci_test_points[index].z)**2)**0.5 == pytest.approx(0, abs=3)

    def test_ecef_to_lla(self):
        for index in range(len(ecef_test_points)):
            lla_test = ecef_test_points[index].lla
            assert lla_test.latitude == pytest.approx(lla_test_points[index].latitude, abs=1e-5)
    ...
    def test_lla_to_ecef(self):
        for index in range(len(lla_test_points)):
            ...
            assert ((ecef_test.x - ecef_test_points[index].x)**2 + (ecef_test.y - ecef_test_points[index].y)**2 + (ecef_test.z - ecef_test_points[index].z)**2)**0.5 == pytest.approx(0, abs=1e-9)
