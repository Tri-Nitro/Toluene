...
void compute_nutation_arguments(double tt_seconds, double nutation_arguments[]) {
    ...
    compute_polynomial(gen_p, t, 5, nutation_critical_arguments+8);
    compute_polynomial(l_, t, 4, nutation_critical_arguments+9);
    compute_polynomial(l_prime, t, 4, nutation_critical_arguments+10);
    compute_polynomial(f_, t, 4, nutation_critical_arguments+11);
    compute_polynomial(d_, t, 4, nutation_critical_arguments+12);
    compute_polynomial(omega, t, 4, nutation_critical_arguments+13);
    ...
    compute_polynomial(EPSILON_a, t, 5, nutation_arguments+2);
...
void lookup_closest_delta_t(double tt_seconds, double* delta_t) {
    ...
    *delta_t = delta_t_list[(delta_t_length-1)*2+1];
...
void compute_terrestrial_matrix(double tt_seconds, double equation_of_the_equinoxes, double terrestrial_matrix[]) {
    ...
    double delta_t;
    lookup_closest_delta_t(tt_seconds, &delta_t);
    double Dut = (tt_seconds-delta_t)/86400.0;
    double delta_t_days = delta_t/86400.0;

    double gmst_term;
    compute_polynomial(gmst_coefficients, Dut, 5, &gmst_term);

    double gast = fmod(gmst_term + gmst_coefficients[6] * delta_t_days + equation_of_the_equinoxes/15.0, 86400.0);
